...
int main()
{
    ...
    while (1)//Keep showting the menu for 7 options
    {
        ...
        {
            ifstream InF("StudentsRecords.txt",ios::in);//Open the file
            if(!InF.is_open())//Show error if it can't be open
            {
            ...
            while(!InFile.eof()) //Read the data from the file
            {
                ...
                for(int j=0; j<tmpstu.number; j++)
                {
                    ...
                    InFile >> tmpstu.ks[j].CourseName >> tmpstu.ks[j].mark;
                }
        ...
        else if (input == 2)//Add a new student
        {
            ...
            cin >> tmpstu.name >> tmpstu.number >> tmpstu.id;
